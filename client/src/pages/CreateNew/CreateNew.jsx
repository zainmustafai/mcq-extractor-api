...
const CreateNew = () => {
    ...
    return (
        <div>
                        ...
                        Array.isArray(MCQs) ? (
                            MCQs.map((MCQ, index) => (
                                <section
                                    ...
                                    <p>{MCQ.explanation}</p>
                                    <button type='button' className='bg-gray-300 hover:bg-blue-500 transition-all duration-150 border-2 border-black p-2 m-2 w-full'
                                        onClick={() => {
                                            setMCQs(MCQs.filter((mcq, i) => i !== index))
                                        }}
                                    >REMOVE IT. It's Dirty Data.</button>
                                </section>
                            ))
                        ) : (
                            <p>No MCQs found.</p>
            ...
            <button type='submit' onClick={handleSubmit} className='border-2 border-black p-5 m-2 w-full bg-yellow-300 hover:bg-red-800 duration-150 transition-colors '>SUBMIT ALL</button>
